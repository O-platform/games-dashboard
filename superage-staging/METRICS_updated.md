...
**Important — date anchor**: effective entry date = `LEAST(sa.acquisition_date::date, sub.date_joined::date)`. When `acquisition_date` is a real backdated date (CPL partners who supply historical lists) it is earlier than `date_joined` and wins. When it is a later batch-import timestamp (confirmed for Meta: all 32K rows carry import dates within the last 90 days despite subscribers going back to Dec 2025) `date_joined` is earlier and wins automatically, correcting the spurious 100% retention. Last updated: 2026-05-21.

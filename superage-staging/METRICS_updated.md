...
**Important — date anchor**: `days_to_unsub` and `cohort_age_days` both use `sub.date_joined::date`, **not** `sa.acquisition_date`. `acquisition_date` in `subscriber_acquisition` is a batch-import timestamp for many sources (confirmed for Meta: all 32 K rows carry import dates within the last 90 days despite subscribers going back to Dec 2025), so using it as the cohort entry date produces spuriously high retention rates. Last updated: 2026-05-21.

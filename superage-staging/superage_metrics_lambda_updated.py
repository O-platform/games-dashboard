...
def lambda_handler(event, context):
    ...
    try:
        ...
        def fetch_acquisition_rows(fallback_label: str, since_days=None, limit: int = 12):
            ...
            # Effective join date: COALESCE(sa.acquisition_date, s.date_subscribed).
            # Taboola rows are excluded from results (canonical label = 'Taboola').
            eff_date_filter = ""
            click_filter = ""
            if since_days is not None:
                eff_date_filter = (
                    f"AND COALESCE(sa.acquisition_date::date, COALESCE(s.date_subscribed, s.date_joined)::date)"
            ...
            cur.execute(f"""
                ...
                s AS (
                    SELECT
                        ...
                        s.state,
                        COALESCE(sa.acquisition_date, COALESCE(s.date_subscribed, s.date_joined)::date) AS eff_date,
        ...
        cur.execute(f"""
            ...
            FROM (
                SELECT
                    ...
                    -- subscribers table even after date_subscribed gets updated to the
        ...
        cur.execute(f"""
            ...
            s AS (
                SELECT
                    ...
                    -- time); fall back to date_subscribed when absent.
                    -- Gate date_unsubscribed by state='Unsubscribed' — resubscribed
                    -- subs keep the OLD date_unsubscribed in the subscribers row even
                    -- after date_subscribed gets bumped to the new join date, which would
                    ...
                    ((CASE WHEN sub.state = 'Unsubscribed' THEN sub.date_unsubscribed::date END)
                        - COALESCE(sa.acquisition_date::date, COALESCE(sub.date_subscribed, sub.date_joined)::date)) AS days_to_unsub,
                    (CURRENT_DATE
                        - COALESCE(sa.acquisition_date::date, COALESCE(sub.date_subscribed, sub.date_joined)::date))::integer AS cohort_age_days
        ...
        cur.execute(f"""
            ...
            s AS (
                SELECT
                    ...
                    COALESCE(sa.acquisition_date, COALESCE(sub.date_subscribed, sub.date_joined)::date) AS eff_joined,
